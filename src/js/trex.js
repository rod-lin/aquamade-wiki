...
window.trex = (function (com) {
	...
	trex.modal = function (config) {
		...
		var id = uid++;
		var main = $("<div class='com-trex-modal ui small modal com-trex-modal-" + id + "'></div>");

		var tr;

		function destroy() {
			if (tr) {
				$("*").not("com-trex-modal-" + id).off("mousedown", tr.pause);
				main.off("click", tr.resume);
				tr.destroy();
			}
		...
		if (config.use_dragi) {
			main.removeClass("ui small modal").dragi({
					height: "auto",
					width: 450,
					title: "T-Rex Runner",
					resize: false,
					...
					onOpen: function () {
						tr = trex.init(main);
						$("*").not("com-trex-modal-" + id).mousedown(tr.pause);
						main.click(tr.resume);
					},
	...
	trex.init = function (cont, config) {
		cont = $(cont);
		...
		mod.saveScore = function () {
			var high = runner.getHighScore();
	
			if (high)
				foci.setLocal("trex-high-score", high);
		};
	...
	var Runner = (function () {
		...
		function Runner(outerContainerId, opt_config) {
			...
			this.outerContainerEl = typeof outerContainerId === "string" ?
									document.querySelector(outerContainerId) : outerContainerId;
		...
		Runner.prototype = {
			...
			loadImages: function () {
				...
				if (IS_HIDPI) {
					sprite.src = resources.x2;
					Runner.imageSprite = sprite;
					this.spriteDef = Runner.spriteDefinition.HDPI;
				} else {
					sprite.src = resources.x1;
			...
			playIntro: function () {
				if (!this.activated && !this.crashed) {
					...
					// this.containerEl.style.width = Trex.config.WIDTH + "px";
			...
			gameOver: function () {
				...
				if (this.distanceRan > this.highestScore) {
					...
					// store score
					foci.setLocal("trex-high-score", this.highestScore);
				}
